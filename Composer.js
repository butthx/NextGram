...
var Composer = class {
  constructor(...middleware) {
    MakePrivate(
      this,
      'handler',
      middleware.length === 0 ? pass : middleware.map(flatten).reduce(concat)
    );
    this.prefix = '.!/';
  }
  middleware() {
    ...
    let filter = (ctx) => {
      for (let cmd of t) {
        ...
        if (typeof cmd == 'string') {
          ...
          return r.test(text || caption);
        }
        if (cmd instanceof RegExp) {
          return cmd.test(text || caption);
        }
        return false;
      }
      return false;
    };
    return this.on(['message:text', 'message:caption']).filter(filter, ...middleware);
  }
